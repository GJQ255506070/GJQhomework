...
class EmperorGame {
    constructor() {
        ...
        if (this.loadGame()) {
            ...
            this.renderConcubineList(); // 确保加载后更新后宫列表
        }
    }
    ...
    // 新增方法：更新妃嫔宠爱值显示
    updateConcubineFavorDisplay() {
        const concubineCards = document.querySelectorAll('.concubine-card');
        concubineCards.forEach((card, index) => {
            if (index < GameState.harem.length) {
                const concubine = GameState.harem[index];
                const favorFill = card.querySelector('.favor-progress-fill');
                const favorText = card.querySelector('.concubine-details');
                
                if (favorFill) {
                    favorFill.style.width = `${Math.min(concubine.favor, 100)}%`;
                }
                
                if (favorText) {
                    // 更新文本显示
                    const detailsDiv = card.querySelector('.concubine-details');
                    if (detailsDiv) {
                        const parts = detailsDiv.textContent.split('|');
                        if (parts.length > 1) {
